...
std::optional<AxisAlignedBoundingBox> Mesh::get_box() { return this->box; }

void Mesh::set_box(float min_x, float max_x, float min_y, float max_y,
...
void Mesh::bind_buffer() {
  ...
  glBufferData(GL_ELEMENT_ARRAY_BUFFER,
               face_indices.size() * sizeof(TriangleVerticeIndex),
               face_indices.data(), GL_STATIC_DRAW);
}

void Mesh::process_rendering(Shader& shader, Camera camera,
                             glm::vec3 lightPos) {
  auto projection =
  ...
  if (this->strip_flag) {
    glDrawElements(GL_LINE_STRIP, face_indices.size() * 3, GL_UNSIGNED_INT,
                   nullptr);
  } else {
    glDrawElements(GL_TRIANGLES, face_indices.size() * 3, GL_UNSIGNED_INT,
                   nullptr);
  }
