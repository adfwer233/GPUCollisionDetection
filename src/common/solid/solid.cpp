...
Mesh Ball::construct_mesh() {
    ...
    for (int i : std::views::iota(0, theta_segments + 1)) {
        for (int j : std::views::iota(0, phi_segments + 1)) {
            ...
            model.vertices.push_back({{x * radius, y * radius, z * radius},
                                      {x, y, z},
    ...
    for (unsigned int i = 0; i < theta_segments; i++) {
        for (unsigned int j = 0; j < phi_segments; j++) {
            ...
            model.face_indices.push_back({ind(i, j), ind(i + 1, j), ind(i + 1, j + 1)});
            model.face_indices.push_back({ind(i, j), ind(i + 1, j + 1), ind(i, j + 1)});
        }
    }
...
Ball::Ball(glm::vec3 t_center, float t_mass, glm::vec3 t_velocity, float t_radius) : radius(t_radius) {
