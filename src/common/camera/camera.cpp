...
void Camera::process_keyboard(CameraMovement direction, float deltaTime) {
  ...
  if (direction == FORWARD) position += camera_front * velocity;
  if (direction == BACKWARD) position -= camera_front * velocity;
  if (direction == LEFT) position -= camera_right_axis * velocity;
  if (direction == RIGHT) position += camera_right_axis * velocity;
  if (direction == DOWN) position -= camera_up_axis * velocity;
  if (direction == UP) position += camera_up_axis * velocity;
...
void Camera::update_camera_vectors() {
  ...
  camera_right_axis = glm::normalize(glm::cross(
      camera_front, world_up));  // normalize the vectors, because their length
