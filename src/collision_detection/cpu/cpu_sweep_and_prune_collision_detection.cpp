...
void CPUSweepAndPruneCollisionDetection::collision_detection(
     ...
     */
    std::ranges::sort(solid_infos, [](SolidInfo &s1, SolidInfo &s2) -> bool { return s1.box.min_x < s2.box.min_x; });
    ...
    for (size_t i = 0; i < solid_ref_vector.size(); i++) {
        auto it = std::upper_bound(solid_infos.begin(), solid_infos.end(), solid_infos[i].box.max_x,
