 ...
 *
 * @note if two ball collide, we will compute the time of collision and compute
 ...
 */
void CPUSweepAndPruneCollisionDetection::collision_detection(
  ...
  for (size_t i = 0; i < solid_ref_vector.size(); i++) {
    auto it = std::upper_bound(
        solid_infos.begin(), solid_infos.end(), solid_infos[i].box.max_x,
        [](float x, SolidInfo &y) -> bool { return x < y.box.min_x; });
   ...
   */
  std::vector<std::vector<size_t>> aabb_collision_result(
      solid_ref_vector.size());
  for (size_t i = 0; i < solid_ref_vector.size(); i++) {
    auto box_i = solid_infos[i].box;
    for (size_t j = i + 1; j < possible_collision_upper_bound[i]; j++) {
      auto box_j = solid_infos[j].box;
      bool overlap_flag = false;
      if (std::max(box_i.min_x, box_j.min_x) <=
          std::min(box_i.max_x, box_j.max_x))
        overlap_flag = true;
      if (std::max(box_i.min_y, box_j.min_y) <=
          std::min(box_i.max_y, box_j.max_y))
        overlap_flag = true;
      if (std::max(box_i.min_z, box_j.min_z) <=
          std::min(box_i.max_z, box_j.max_z))
        overlap_flag = true;
