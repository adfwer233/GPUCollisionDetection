...
//        printf("initialized %d %d %d %d\n", blockIdx.x, partial, input[global_tid * ITEMS_PER_THREAD], blockDim.x);
...
//                printf("rel %d %d %d %d %d\n", blockIdx.x, j, exclusive, tile_inclusive[j], tile_status[j]);
            }
    ...
    for (int i = 0; i < ITEMS_PER_THREAD; i++) {
        if (threadIdx.x > 0)
            output[global_tid * ITEMS_PER_THREAD + i] = items[i] + exclusive + block_sum[threadIdx.x - 1];
        else
            output[global_tid * ITEMS_PER_THREAD + i] = items[i] + exclusive;
    }
}

__global__ void scan_init(int n) {
    initialize_status(n);
}

__global__ void scan(int *input, int *output) {
    consume_tile(input, output);
}
