...
void GPUSweepAndPruneCollisionDetection::collision_detection(
  ...
  find_possible_collision_upper_bound_kernel<<<int(n / (thread_per_block * ITEMS_PER_THREAD)) + 1, thread_per_block>>>(
  ...
  find_aabb_collision_pairs_kernel<<<int(n / (thread_per_block * ITEMS_PER_THREAD)) + 1, thread_per_block>>>(
      dev_solid_info_sorted,
      possible_collision_upper_bound,
      aabb_collision_result, n);

  /*
   * Step5: collision detection between balls
   */

  ball_collision_detection_kernel<<<int(n / (thread_per_block * ITEMS_PER_THREAD)) + 1, thread_per_block>>>(dev_solid_info_sorted, aabb_collision_result, gpu_collision_result, n);

  cudaMemcpy(host_gpu_collision_result, gpu_collision_result, n * MAX_COLLISION_SOLID_COUNT * sizeof(size_t ), cudaMemcpyDeviceToHost);
