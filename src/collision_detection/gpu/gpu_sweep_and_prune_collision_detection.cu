...
void GPUSweepAndPruneCollisionDetection::collision_detection(
    ...
    cudaMalloc(&dev_solid_info_sorted, n * sizeof(decltype(solid_infos)::value_type));
    cudaMemcpy(dev_solid_info, solid_infos.data(), n * sizeof(SolidInfoGPU), cudaMemcpyHostToDevice);


    struct SolidInfoDecomposerT {
        __host__ __device__ cuda::std::tuple<float &> operator() (SolidInfoGPU &key) {
    ...
    cub::DeviceRadixSort::SortKeys(d_temp_storage, temp_storage_bytes, dev_solid_info, dev_solid_info_sorted, n, SolidInfoDecomposerT{});
    ...
    find_possible_collision_upper_bound_kernel<<<256, 64>>>(dev_solid_info_sorted, possible_collision_upper_bound, n);
    ...
    find_aabb_collision_pairs_kernel<<<256, 64>>>(aabb_collision_result, possible_collision_upper_bound, n);

    /*
     * Step5: collision detection between balls
     */
    thrust::device_vector<std::pair<size_t, size_t>> gpu_collision_result;

    ball_collision_detection_kernel<<<256, 64>>>(aabb_collision_result, gpu_collision_result, dev_solid_info_sorted, n);
}