...
__global__ void find_possible_collision_upper_bound_kernel(
    ...
    for (int i = 0; i < ITEMS_PER_THREAD; i++) {
        ...
        if (id < n) {
//            auto ptr_info = thrust::raw_pointer_cast(solid_info.data() + id);
            auto it = upper_bound(solid_info, solid_info + n, solid_info[id].max_x, [](float x, SolidInfoGPU &y){return x < y.min_x;});
...
__global__ void find_aabb_collision_pairs_kernel(
    SolidInfoGPU *solid_info,
    size_t *possible_upper_bound,
    size_t *result,
    int n) {
    ...
    for (size_t i = 0; i < ITEMS_PER_THREAD; i++) {
        ...
        for (size_t j = id + 1; j < upper_bound; j++) {
            if (collision_count >= MAX_COLLISION_SOLID_COUNT) break;
...
__global__ void ball_collision_detection_kernel(SolidInfoGPU *solid_info, const size_t *aabb_collision, size_t *result, int n) {
    size_t tid = blockIdx.x * blockDim.x + threadIdx.x;
    for (size_t i = 0; i < ITEMS_PER_THREAD; i++) {
        size_t id = tid * ITEMS_PER_THREAD + i;
        ...
        for (int j = 0; j < MAX_COLLISION_SOLID_COUNT; j++) {
            if (aabb_collision[id * MAX_COLLISION_SOLID_COUNT + j] == id) break;
            ...
            if ((i_radius + j_radius) * (i_radius + j_radius) > (i_center_x - j_center_x) * (i_center_x - j_center_x) + (i_center_y - j_center_y) * (i_center_y - j_center_y) + (i_center_z - j_center_z) * (i_center_z - j_center_z)) {
